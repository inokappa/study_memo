 ...
 * Chef だと cookbook にあたるものが Ansible だと Playbook と呼ばれる YAML で記述されたファイルとなる
...
TASK: [install mysql-server] ************************************************** 
ok: [ec2-xx-xxx-xx-xx2.ap-northeast-1.compute.amazonaws.com] => {"changed": false, "msg": "", "rc": 0, "results": ["mysql-server-5.5-1.3.amzn1.noarch providing mysql-server is already installed"]}
changed: [ec2-54-250-11-55.ap-northeast-1.compute.amazonaws.com] => {"changed": true, "msg": "", "rc": 0, "results": （略）\n\n"]}

TASK: [mysql is running] ****************************************************** 
ok: [ec2-xx-xxx-xx-xx2.ap-northeast-1.compute.amazonaws.com] => {"changed": false, "name": "mysqld", "state": "started"}
changed: [ec2-54-250-11-55.ap-northeast-1.compute.amazonaws.com] => {"changed": true, "name": "mysqld", "state": "started"}
...
serverspec の @gosukenator さんが serverspec の並列処理についてツイートされていたので、早速、並列処理で serverspec を実行させてみた。
